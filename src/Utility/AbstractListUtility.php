...
abstract class AbstractListUtility
{
    ...
    {
        return array_filter($list, function(array $item) use ($idsToRemove) {
    ...
    public static function moveMultipleBetween(array $list, string $previousId, string $nextId, array $ids): array
    {
        ...
        while (!empty($previousIds)) {
            ...
            while (!empty($nextIds)) {
                ...
                array_push($currentNextIds, $currentNextId);
            }
            ...
            array_push($currentPreviousIds, $currentPreviousId);
        }
